...
internal struct BasicModelResponder<Model>: AnyModelResponder where Model: FluentKit.Model {
    private let _handle: @Sendable (ModelEvent, Model, Database) throws -> EventLoopFuture<Void>
    ...
    init(handle: @escaping @Sendable (ModelEvent, Model, Database) throws -> EventLoopFuture<Void>) {
        self._handle = handle
    }
}
